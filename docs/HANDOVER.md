...
## TL;DR
3D-Entenangel-Lernspiel (Three.js + Vite + TS strict). **M0–M4 fertig & gepusht**; **M4.6 (Game-Feel & Bright-Comic-Overhaul) Steps 1–6 gepusht** (Comic-Tag, Toon+Outline-Enten, **Fang-Engine-Neumodell**: feste Schräg-Sicht aufs ganze Becken, Maus → Wasserpunkt W, Halten senkt den Haken **ins Wasser**, **räumlicher Fang** (Ente nahe W; Perfect = mittig), Rute schwenkt stark, Rute/Haken Toon+Outline). **AKTUELL offen:** richtige Jahrmarkt-Welt (graue Wand + dicker Reifen weg), Juice, schickere Tipp-Modals. **Siehe Abschnitt „AKTUELL: M4.6" unten — höchste Priorität.** Repo: https://github.com/Scholzer0303/quack-and-catch (öffentlich).

## Session-Start-Routine (Pflicht)
```bash
git log --oneline -15   # zuletzt: M4.6 Step 6 = Fang-Engine-Neumodell (raeumlich, ins Wasser) + Rute-Optik
...
src/ui/                 ← Reticle (Fadenkreuz am Wasserpunkt + Farb-/Dip-Feedback), UIRoot, HUD, StartScreen,
...
1. ~~**Steuerungs-Redesign**~~ ✅ (Step 5) + ~~**Fang-Engine-Neumodell + Rute/Haken-Optik**~~ ✅ (Step 6). Engine ist jetzt **räumlich** (siehe Gotcha „M4.6-Fang-Engine" unten): Maus → Wasserpunkt W, Halten senkt den Haken ins Wasser, Loslassen mit Ente ≤ `catchRadius` um W fängt (Perfect = mittig). Feste Schräg-Sicht aufs ganze Becken, Rute schwenkt stark (yaw/pitch), Rute/Haken Toon+Outline + Schwimmer.
2. **Jahrmarkt-Welt (NÄCHSTER SCHRITT)** (`StallBuilder.ts`: Buden/Wimpel/Kulisse statt grauer Wand; `BasinBuilder.ts`: Rand statt dickem Reifen). Optional Bloom/Glow (Design unten).
3. **Juice** (Design unten): Splash, Catch-Pop, Perfect-Flash, Mini-Screenshake, HUD-Count-up.
4. **Tipp-Modal** schicker (`CardReveal.ts` + `styles.css` `.qc-card*`); optional `icon`-Feld in `data/tips.ts`.
5. **Optional:** Intro-Sequenz (neuer Screen + Phase `intro` in `GameStateMachine`).
...
## Roadmap-Rest
**M4.6 Game-Feel & Comic-Overhaul (IN ARBEIT, Steps 1–6 ✅, Rest = Jahrmarkt-Welt/Juice/Modals)** → M4.5 Vercel-Live-Deploy → M5 Codex → M6 Shop → M7 Progression → M8 Audio (+ Rest-Juice) → M9 Stretch. Tipp-Codex: ~50–60 eigene, faktisch geprüfte deutsche Karten (12 seit M3).
